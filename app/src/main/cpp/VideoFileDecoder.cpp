...
bool VideoFileDecoder::initComponents(const char *path) {
    ...
    if (audioIndex == -1 && videoIndex == -1) {
        LOGE("Neither audio stream or video stream found in the file");
        return false;
    }
    ...
    if (audioIndex != -1) {
        ...
        LOGE(" in_channels:%d\n", in_channels);
        LOGE(" in_channels_layout:%ld\n", in_channel_layout);

        audioSwrCtx = swr_alloc();
        audioSwrCtx = swr_alloc_set_opts(audioSwrCtx, out_channel_layout, out_sample_fmt,
                                         out_sample_rate, in_channel_layout, in_sample_fmt,
    ...
    if (videoIndex != -1) {
        ...
        videoSwsCtx = sws_getContext(videoCodecCtx->width, videoCodecCtx->height,
                                     videoCodecCtx->pix_fmt, videoCodecCtx->width,
                                     videoCodecCtx->height, AV_PIX_FMT_RGB24, SWS_BILINEAR, NULL,
                                     NULL, NULL);
        if (videoSwsCtx == NULL) {
...
void VideoFileDecoder::closeInput() {
    ...
    LOGE("decodeAudioThread stopped");
    ...
    LOGE("decodeVideoThread stopped");
...
void VideoFileDecoder::removeDataReceiver(IMediaDataReceiver *receiver) {
    ...
    while (!stopDecodeFlag && !readFinish) {
        ...
        if (av_read_frame(formatCtx, packet) < 0) {
            ...
            if (packet->stream_index == audioIndex) {
                LOGD("read a audio packet,put it to queue,audioPacketQueue.size = %d",
                     audioPacketQueue->getSize());
                audioPacketQueue->put(packet);
                LOGD("put audio packet finished");
            } else if (packet->stream_index == videoIndex) {
                LOGD("read a video packet,put it to queue,videoPacketQueue.size = %d",
...
void VideoFileDecoder::decodeAudio() {
    ...
    while (!stopDecodeFlag && !readFinished) {
        ...
        if (err == AVERROR(EAGAIN)) {
            ...
            //codec says isEOF,cause we set the packat->size= 0.
            readFinished = true;
        } else if (err != 0) {
            ...
            while (1) {
                ...
                if (err == AVERROR(EAGAIN)) {
                    ...
                    audioFrame->pts = (int64_t) (frame->pts * av_q2d(audioStream->time_base) *
                                                 1000);
                    uint8_t *tempData = (uint8_t *) (audioFrame->data);
                    audioFrame->sampleCount = swr_convert(audioSwrCtx, &(tempData),
                                                          audioSampleCountLimit,
...
void VideoFileDecoder::decodeVideo() {
    ...
    av_image_fill_arrays(convertedFrame->data, convertedFrame->linesize,
                         outBuffer, AV_PIX_FMT_RGB24, videoCodecCtx->width, videoCodecCtx->height,
                         1);
    bool readFinish = false;
    int err = 0;
    while (!stopDecodeFlag && !readFinish) {
        ...
        if (err == 0) {
            ...
            chrono::milliseconds decodeDuration = chrono::duration_cast<chrono::milliseconds>(
                    endTime - startTime);
        ...
        if (err == AVERROR(EAGAIN)) {
            ...
            while (1) {
                ...
                } else // err == 0
                {
                    ...
                    videoFrame->pts = (int64_t) (frame->pts * av_q2d(videoStream->time_base) *
                                                 1000);
                    err = sws_scale(videoSwsCtx, (const uint8_t *const *) frame->data,
                                    (const int *) frame->linesize, 0, videoCodecCtx->height,
