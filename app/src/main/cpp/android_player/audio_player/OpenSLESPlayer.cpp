...
void OpenSLESPlayer::processAudio() {
    ...
    if (provider != NULL) {
        ...
        if (data == NULL) {
            ...
            (*playerBufferQueue)->Enqueue(playerBufferQueue, emptyBuffer,
                                          EMPTY_BUFFER_SAMPLES * 2 * sizeof(int16_t));
        } else {
            memcpy(emptyBuffer, data->data, data->sampleCount * 2 * sizeof(int16_t));
            (*playerBufferQueue)->Enqueue(playerBufferQueue, emptyBuffer,
                                          data->sampleCount * 2 * sizeof(int16_t));
...
bool OpenSLESPlayer::create() {
    ...
    SLDataLocator_AndroidSimpleBufferQueue bufferQueue = {SL_DATALOCATOR_ANDROIDSIMPLEBUFFERQUEUE,
                                                          2};
    SLDataFormat_PCM pcmFormat = {SL_DATAFORMAT_PCM, 2, SL_SAMPLINGRATE_44_1,
    ...
    result = (*engineEngine)->CreateAudioPlayer(engineEngine, &playerObject, &audioSrc, &audioSink,
                                                2, ids2, reqs2);
    if (result != SL_RESULT_SUCCESS) {
        return false;
    }
